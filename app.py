...
image = (
    Image.debian_slim().apt_install("git", "ffmpeg", "curl", "mecab", "mecab-ipadic-utf8", "libmecab-dev", "swig")
    .run_commands(
        ["git clone https://github.com/myshell-ai/MeloTTS.git",
             ...
             "pip install -e .",
             "python -m unidic download",]).run_commands(["python -m pip install wheel", "pip install faster_whisper"]).env({"HF_HUB_ENABLE_HF_TRANSFER": "1"}).run_commands(["pip install hf_transfer"]).run_function(
        download_model
    ))
...
@stub.function(cpu=8.0, memory=16384, keep_warm=2)
@asgi_app()
def entrypoint():
    import torch
    from faster_whisper import WhisperModel
